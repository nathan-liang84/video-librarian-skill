...
| 代码编辑(机械层) | **Atlas(MiniMax M3 / OpenClaw)** | 低风险、规格清晰、爆炸半径小的机械任务:格式兼容纯函数、IO 工具、环境探测、测试脚手架、文档/配置。**不碰**状态机/数据契约/改名/脚本匹配等高风险模块 |
| 代码检查 | **GPT-5.5(extra high)** | **复审 Opus 写的全部模块**;以及 Codex / Atlas PR 的全量 review |
| 代码检查(高风险) | **Opus 4.8** | 复审 Codex / Atlas 写的高风险模块(见下表 ⚠️);**不审自己写的** |

> 评审第一原则:**审查者 ≠ 作者**。
> - Codex 写的 → GPT-5.5 全量 + Opus 终审高风险项。
> - **Atlas(M3)写的 → Opus 或 GPT-5.5 复审**(Atlas 不自审)。M3 作为编码模型尚在试用,**暂不作为任何模块的终审人**;其任务须规格清晰、不触碰契约/状态机/改名。试用稳定后再考虑放权到 Codex 同级。
> - **Opus 写的 → GPT-5.5(extra high)复审**(Opus 不自审)。评审清单见 [REVIEW_CHECKLIST.md](REVIEW_CHECKLIST.md)。

> **任务分级速记**:契约/状态机/改名/匹配/提示词 = 高风险 → Opus 写、GPT-5.5 终审;常规管线工程 = Codex;隔离纯函数 / IO / 探测 / 测试 / 文档 = Atlas(机械层)。三者写的都不由自己终审。
