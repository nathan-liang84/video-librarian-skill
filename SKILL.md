...
1. **系统依赖**:`ffmpeg` / `ffprobe`(视频抽帧、抽音轨)。先跑 `scripts/00_detect_env.py` 自检(会按你的系统给出对应安装命令)。
   - macOS:`brew install ffmpeg`;Linux:`apt/dnf install ffmpeg`;**Windows**:`winget install ffmpeg`(或 choco/scoop),装后确保在 PATH。
2. **Python 依赖**:`pip install -r requirements.txt`(跨平台;Windows 建议先建 venv,**不需要** `--break-system-packages`)。
3. **配置**:复制 `config/config.example.yaml` 为 `config/config.yaml`,填写:
   - 模型:你自配的多模态(看画面)与文本模型的 API key
   - ASR(可选):本地 `faster-whisper` 模型档位;不装则跳过语音转写,视频仍抽帧理解
...
模型与人物**都不预设**:本 skill 走通用 OpenAI 兼容接口,且**不内置任何主角**。
首次使用时,Agent 应**主动引导用户完成配置**,而不是假设默认值或套用任何示例里的人名:

1. **选"看画面"模型(必须支持图像输入)**:任选一个支持图像输入的多模态模型(任意 OpenAI 兼容 provider 的视觉模型,或本地多模态)。强调:**纯文本模型不能当 vision 用**。
2. **选"处理文本"模型**:任意文本模型,可与上面同家或换更便宜的。
...
5. **建人物名册(必须问用户,切勿假设)**:
   - **不要预设任何主角名**。主动问用户:"你这套素材的主角是谁?叫什么名字?"——名字由用户给。
   - 请用户**上传主角参考图**到 `config/refs/`,命名为 `<主角名>.jpg`(可多张:`<主角名>_1.jpg`、`<主角名>_背影.jpg`、`<主角名>_全身.jpg`、`<主角名>_穿搭1.jpg`,有助没露脸时识别)。
   - 有固定出镜的关系人/宠物也照此加进 `companions` 并放对应参考图。
   - 把这些写进 `config/config.yaml` 的 `people`(`config.example.yaml` 里 `name` 默认留空,等用户填)。
...
| 3 | `03_understand.py` | 多模态模型看帧(+主角参考图,含背影/穿搭)+ 文本模型融合 → 结构化内容、受控标签、可用片段、人物(含 subject_confidence/basis) |
...
- **每条都有主体**:`main_subject`(+`subject_kind`:人物/物品/建筑/风景/动物/食物/其他)由模型判断画面主次——焦点是人用名册名,否则给物品/建筑/风景词(柠檬茶/教堂/海岸线)。文件名主体段 `{subject}` 即取它,没人入镜也不会丢主体。`subjects`(人物,供匹配)与 `main_subject`(主体,供命名)解耦。
- **没露脸也别漏主角**:① 参考图除正脸外,放几张背影/全身/常穿搭,多模态模型会用非面部特征认人;② 开 `people.bias_to_main` 后,有人但拿不准、又无法排除是主角时记为主角(低 `subject_confidence` + `subject_basis=inferred`)并自动 `needs_review`,由 `review.py` 人工一键确认,绝不悄悄漏成「多人」。
