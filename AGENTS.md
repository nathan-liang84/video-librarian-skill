...
> **本文件面向所有协作者**(Opus / Codex / Atlas / GPT-5.5 + 用户)。
> **进入仓库/新对话第一件事**:读完本文件 → 在 §2 对号入座找到你的角色 → 跑 §7「查当前状态」对齐进度 → 按 §3 规则开工。
> 本文件是协作的唯一权威入口;模块级细分见 [docs/TASK_BREAKDOWN.md](docs/TASK_BREAKDOWN.md)。
...
- **新开的 Claude 对话 → 你是 Opus 4.8(架构师 + 协调者)**:亲自写高风险模块(契约/状态机/改名/匹配/提示词);把常规活拆成 issue 派 Codex、机械活派 Atlas;**终审**他人高风险(不审自己写的);用 `gh` 直接读写 GitHub。
- **从 Codex 界面 / `@codex` 进来 → 你是 Codex(GPT-5.4)**:接常规/集成 issue,按 §4 认领并开发。
- **从 OpenClaw 进来 → 你是 Atlas(MiniMax M3)**:只接机械层 issue,严守 §1 的「不碰」清单。
- **被叫来审 PR → 你是审查方(GPT-5.5 / Opus)**:按 §3 评审矩阵审,**通过即合并**(见 §5⑥)。

不确定时默认按 Opus(协调者)行事;但**不要替别的 agent 写它负责的模块**。各 agent 通过 GitHub issue/PR 异步协作(§4–§6)。

## 3. 协作铁律

1. **不自审;审查通过即自动合并** —— 评审必须由他人完成(作者 ≠ 审查者)。审查方判定通过后,**立即执行合并(`gh pr merge`),不再回到用户手动点合并**;合并动作由审查方或协调者(Opus)执行。作者**不得在无他方通过时合并**自己的 PR(守住"先审后合")。
2. **评审矩阵**:
...
⑤ 回修    你把评论链接转给作者 → 读 comment → 改 → 推 → 回复 comment
            ↑——————————— ④⑤ 循环直到通过 ———————————┘
⑥ 合并    审查通过 → 审查方/Opus 立即 `gh pr merge`(自动合并,不回到用户手动点)
            → Closes #N 自动关 issue
```
