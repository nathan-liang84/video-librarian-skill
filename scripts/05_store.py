...
def main() -> int:
    ...
    # junk(垃圾照片)/ grouped(近重复非代表成员)默认入库为"最小记录"(无理解结果、无新名),
    # 供后置 audit / 经 group_id 发现;is_junk / is_representative=False 是持久标记,06 据此不召回。
    # 状态随后置 stored,保证 05 幂等、不重复入库。
    wanted = {"named", "understood"} if args.include_understood else {"named"}
    wanted.update({"junk", "grouped"})
