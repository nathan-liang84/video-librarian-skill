...
PEOPLE_ON = {"main": {"name": "Alice"}, "bias_to_main": True}
PEOPLE_OFF = {"main": {"name": "Alice"}, "bias_to_main": False}
...
def test_subject_atoms_includes_main_in_composite():
    assert "Alice" in m03._subject_atoms(["Alice和多人"])
    assert "Alice" not in m03._subject_atoms(["多人"])


def test_low_conf_inferred_main_is_tentative():
    # 没露脸、低置信被推断为Alice → 待确认
    assert m03.is_tentative_main(_rec(["Alice和多人"], 0.4), PEOPLE_ON, 0.6) is True


def test_high_conf_main_not_tentative():
    # 看到脸、高置信 → 不需确认
    assert m03.is_tentative_main(_rec(["Alice"], 0.95), PEOPLE_ON, 0.6) is False


def test_bias_off_never_tentative():
    # 未开启先验 → 不触发(保持通用严格行为)
    assert m03.is_tentative_main(_rec(["Alice和多人"], 0.4), PEOPLE_OFF, 0.6) is False
...
def test_none_confidence_not_tentative():
    ...
    assert m03.is_tentative_main(_rec(["Alice"], None), PEOPLE_ON, 0.6) is False
...
def test_inferred_basis_without_confidence_is_tentative():
    ...
    assert m03.is_tentative_main(
        _rec(["Alice和多人"], None, basis="inferred"), PEOPLE_ON, 0.6) is True


def test_appearance_basis_without_confidence_is_tentative():
    # 靠外观(发型/穿搭)认出但没给可信度 → 同样保守送审。
    assert m03.is_tentative_main(
        _rec(["Alice"], None, basis="appearance"), PEOPLE_ON, 0.6) is True


def test_high_conf_inferred_still_tentative_when_below_thresh():
    # inferred 即便给了可信度,低于阈值照样送审。
    assert m03.is_tentative_main(
        _rec(["Alice"], 0.5, basis="inferred"), PEOPLE_ON, 0.6) is True
    # 高于阈值的 inferred(模型很有把握)→ 放行,尊重模型可信度。
    assert m03.is_tentative_main(
        _rec(["Alice"], 0.8, basis="inferred"), PEOPLE_ON, 0.6) is False


def test_resolve_people_preserves_bias_and_hint():
    cfg = {"people": {"main": {"name": "Alice"}, "bias_to_main": True,
    ...
    assert resolved["main"]["name"] == "Alice"
